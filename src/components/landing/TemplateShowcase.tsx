...
function TemplatePreview({ id, name }: { id: string; name: string }) {
  try {
    return (
      <img
        src={`/studio/template-previews/${id}.jpg`}
        alt={name}
...
export function TemplateShowcase({ templates }: { templates: TemplateDefinition[] }) {
  ...
  const filtered = useMemo(() => templates.filter((t) => t.id !== 'mobile-canvas'), [templates]);

  return (
    <section className="py-20 sm:py-28 bg-gradient-to-b from-white via-zinc-50/30 to-white relative overflow-hidden">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold font-[var(--typography-font-heading)] text-zinc-900">
            {filtered.length} Templates, Endless Character
          </h2>
          ...
          {filtered.map((t) => {
            ...
            return (
              <button
                ...
                className="group text-left bg-white rounded-2xl border border-zinc-200 overflow-hidden hover:border-[var(--colors-primary)]/40 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                <div
                  className="relative h-44 flex items-end p-4 overflow-hidden"
                  style={{ background: `linear-gradient(135deg, ${a} 0%, ${b} 100%)` }}
                >
                  <div className="absolute inset-0 opacity-15" style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.7) 1px, transparent 1px)', backgroundSize: '16px 16px' }} />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
                  <span className="absolute top-3 right-3 text-[10px] font-medium px-2 py-0.5 rounded-full bg-white/85 text-zinc-700 backdrop-blur-sm z-10">
                    {cat}
                  </span>
                  <span className="relative z-10 font-[var(--typography-font-heading)] text-white text-xl font-semibold drop-shadow-sm tracking-tight">
                    {t.name}
                  </span>
                </div>
                <div className="p-4">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex gap-1">
                      {['primary', 'secondary', 'accent', 'background', 'text'].map((key) => {
                        const c = t.theme?.colors ? (t.theme.colors as Record<string, string>)[key] : undefined;
                        if (!c) return null;
                        return (
                          <div key={key} className="w-3.5 h-3.5 rounded-full border border-zinc-200" style={{ backgroundColor: c }} title={key} />
                        );
                      })}
                    </div>
                    <span className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--colors-primary)] opacity-0 group-hover:opacity-100 transition-opacity">
                      Use Template
