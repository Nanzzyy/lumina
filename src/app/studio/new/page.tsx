...
export default function NewInvitationPage() {
  ...
  if (step === 'pick') {
    return (
      <div>
        ...
        <p className="text-sm text-zinc-500 mb-8">Select a template to start building your invitation.</p>
            ...
            >
              ...
              <div className="h-32 rounded-lg mb-4 overflow-hidden"
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-white/80 text-2xl font-[var(--typography-font-heading)]">
                    {t.name}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                <span className="text-[10px] px-2 py-0.5 bg-zinc-100 text-zinc-500 rounded-full">{t.sections.length} sections</span>
                <span className="text-[10px] px-2 py-0.5 bg-zinc-100 text-zinc-500 rounded-full">{t.animation?.preset}</span>
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <button onClick={() => setStep('pick')} className="text-sm text-zinc-400 hover:text-zinc-600 mb-6 flex items-center gap-1">
      ...
      <p className="text-sm text-zinc-500 mb-8">Template: <span className="font-medium text-zinc-700">{templates.find((t) => t.id === selectedTemplate)?.name}</span></p>
