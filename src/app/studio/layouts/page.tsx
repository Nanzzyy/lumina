      ...
      {loading ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {layouts.map((l) => (
            <button
              key={l.id}
              onClick={() => router.push(`/studio/layouts/new?edit=${l.id}`)}
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                {l.isBuiltin && (
                  <span className="text-[10px] px-2 py-0.5 bg-zinc-100 text-zinc-400 rounded-full">Built-in</span>
                )}
              </div>
              <p className="text-sm text-zinc-500 mb-4">{l.description}</p>
              <div className="flex flex-col gap-1.5">
                {l.config.sections.map((s, i) => (
