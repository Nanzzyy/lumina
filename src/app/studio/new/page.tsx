...
function NewInvitationContent() {
  ...
  const [step, setStep] = useState<'template' | 'layout' | 'name'>(
    preSelectedTemplate
      ? (templates.find((t) => t.id === preSelectedTemplate)?.kind === 'monolithic' ? 'name' : 'layout')
      : 'template',
  );
  ...
  const handlePickTemplate = (id: string) => {
    ...
    // Monolithic templates render themselves — no layout needed, skip straight to naming.
    const t = templates.find((t) => t.id === id);
    const isMonolithic = t?.kind === 'monolithic';
    const isMobileCanvas = t?.kind === 'mobile-canvas';
    setStep(isMonolithic || isMobileCanvas ? 'name' : 'layout');
  };
  ...
  const handleCreate = async () => {
    ...
    const isMobileCanvas = picked?.kind === 'mobile-canvas';
    // Mobile-canvas and monolithic don't need layout.
    if (!isMobileCanvas && picked?.kind !== 'monolithic' && !selectedLayout) return;
    ...
    const isSolo = picked?.mode === 'solo';
    content.couple = isSolo ? { ...content.couple, partner1: selectedTemplate === 'undangan-birthday-gala' ? 'Gwyneth' : 'Celebrant' } : { ...content.couple, partner1: 'Partner 1', partner2: 'Partner 2' };
    // Mobile-canvas: seed clean slate (no couple data needed for canvas builder)
    if (isMobileCanvas) {
      content.couple = { partner1: '', partner2: '' };
    }

    await create({
      slug: finalSlug,
      templateId: selectedTemplate,
      layoutId: selectedLayout || 'default',
      title: title.trim(),
      content,
    ...
    if (isMobileCanvas) {
      router.push(`/studio/mobile-builder/${finalSlug}`);
    } else if (picked?.category === 'mobile') {
      router.push(`/studio/mobile/${finalSlug}`);
    } else {
      router.push(`/studio/${finalSlug}`);
    }
  };

  const weddingTmpls = templates.filter((t) => !t.category || t.category === 'wedding');
  const eventTmpls = templates.filter((t) => t.category === 'event');
  const mobileTmpls = templates.filter((t) => t.category === 'mobile');
  ...
  if (step === 'template') {
    return (
      <div>
        ...
        <p className="text-sm text-zinc-500 mb-8">Pilih template sesuai acara Anda.</p>

        {weddingTmpls.length > 0 && (
          <>
            <h3 className="text-sm font-semibold text-zinc-600 uppercase tracking-wider mb-3">Pernikahan</h3>
        ...
        {eventTmpls.length > 0 && (
          <>
            <h3 className="text-sm font-semibold text-zinc-600 uppercase tracking-wider mb-3">Acara</h3>
            <div className="grid gap-5 sm:grid-cols-2">
              {eventTmpls.map((t) => (
                <button key={t.id} onClick={() => handlePickTemplate(t.id)} className="text-left bg-white rounded-xl border border-zinc-200 p-6 hover:border-[var(--colors-primary)]/40 hover:shadow-md transition-all group">
                  <div className="h-32 rounded-lg mb-4 overflow-hidden flex items-center justify-center bg-gradient-to-br from-zinc-300 to-zinc-500">
                    <span className="text-white/80 text-2xl font-[var(--typography-font-heading)]">{t.name}</span>
                  </div>
                  <h3 className="font-semibold text-zinc-900 group-hover:text-[var(--colors-primary)] transition-colors">{t.name}</h3>
                  <p className="text-sm text-zinc-500 mt-1">{t.description}</p>
        ...
        {mobileTmpls.length > 0 && (
          <>
            <h3 className="text-sm font-semibold text-zinc-600 uppercase tracking-wider mb-3 mt-10">Mobile</h3>
            <p className="text-xs text-zinc-400 mb-4">Template mobile-first — langsung edit kode HTML/CSS/TSX.</p>
            <div className="grid gap-5 sm:grid-cols-2">
              {mobileTmpls.map((t) => (
                <button key={t.id} onClick={() => handlePickTemplate(t.id)} className="text-left bg-white rounded-xl border border-zinc-200 p-6 hover:border-[var(--colors-primary)]/40 hover:shadow-md transition-all group">
                  <div className="h-32 rounded-lg mb-4 overflow-hidden flex items-center justify-center bg-gradient-to-br from-[#D4A574] via-[#A8B5A0] to-[#E8C4C0]">
                    <span className="text-white/90 text-2xl font-[var(--typography-font-heading)]">{t.name}</span>
                  </div>
                  <h3 className="font-semibold text-zinc-900 group-hover:text-[var(--colors-primary)] transition-colors">{t.name}</h3>
                  <p className="text-sm text-zinc-500 mt-1">{t.description}</p>
                </button>
  ...
  const isMonolithic = pickedTemplate?.kind === 'monolithic';
  const isMobileCanvas = pickedTemplate?.kind === 'mobile-canvas';
  const noLayoutStep = isMonolithic || isMobileCanvas;

  return (
    <div className="max-w-lg mx-auto">
      <button onClick={() => setStep(noLayoutStep ? 'template' : 'layout')} className="text-sm text-zinc-400 hover:text-zinc-600 mb-6 flex items-center gap-1">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        {noLayoutStep ? 'Back to templates' : 'Back to layouts'}
      </button>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <input value={slug} onChange={(e) => setSlug(e.target.value.replace(/[^a-z0-9-]/g, ''))}
              className="flex-1 px-4 py-2.5 rounded-lg border border-zinc-300 text-base sm:text-sm focus:outline-none focus:ring-2 focus:ring-[var(--colors-primary)] focus:border-transparent"
